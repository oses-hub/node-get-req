...
const { request_response } = require('./request');
var slowAES = {
    /*
     * START AES SECTION
     */
...
async function var_a(response) {
    const is = /var a=toNumbers\W\"(.*?)\"\W,/gm
    const finish = is.exec(response)
    return finish[1]
}

async function var_b(response) {
    const is = /b=toNumbers\W\"(.*?)\"\W,/gm
    const finish = is.exec(response)
    return finish[1]
}
async function var_c(response) {
    const is = /c=toNumbers\W\"(.*?)\"\W;/gm
    const finish = is.exec(response)
    return finish[1]
}
...
async function request_to_get_vars(url) {
                    ...
                    var_b(body).then(bs => {
                        var_c(body).then(cs => {
                            var a = toNumbers(as),
                                b = toNumbers(bs),
                                c = toNumbers(cs);
                            var __test = toHex(slowAES.decrypt(c, 2, a, b))
                            request_to_get_page_response(url, __test).then(res => {
                                resolve(res)
                            })
                        })
                    })
...
async function request_to_get_page_response(url, __test) {
    return new Promise(function(resolve, reject) {
        request.get({
            url: url,
            headers: {
                'User-Agent': 'Mozilla/5.0',
                'Cookie': `__test=${__test}`
            }
        }, function(err, httpResponse, body) {
            if (err) {
                reject(err);
